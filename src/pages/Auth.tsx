...
import { Eye, EyeOff, Mail, Lock, User, Scissors, ArrowRight } from "lucide-react";
...
const Auth = () => {
  ...
  const handleSubmit = () => {
    ...
    navigate(selectedRole === "designer" ? "/" : "/client-home");
  };
        ...
        {step === "role" ? (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
              ...
              <motion.button
                ...
                className={`relative rounded-2xl p-5 flex flex-col items-center gap-3 border-2 transition-all duration-200 ${
                  selectedRole === "client"
              ...
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${
                  selectedRole === "client" ? "bg-primary/20" : "bg-secondary"
                }`}>
                  <User className={`w-7 h-7 ${selectedRole === "client" ? "text-primary" : "text-muted-foreground"}`} />
                </div>
                <div className="text-center">
                  <p className="text-sm font-bold text-foreground">Client</p>
                  <p className="text-[10px] text-muted-foreground mt-0.5">Browse & book designers</p>
                </div>
                {selectedRole === "client" && (
                  <motion.div
                    layoutId="check"
              ...
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${
                  selectedRole === "designer" ? "bg-primary/20" : "bg-secondary"
                }`}>
                  <Scissors className={`w-7 h-7 ${selectedRole === "designer" ? "text-primary" : "text-muted-foreground"}`} />
                </div>
                <div className="text-center">
                  <p className="text-sm font-bold text-foreground">Designer</p>
                  <p className="text-[10px] text-muted-foreground mt-0.5">Manage your business</p>
                </div>
                {selectedRole === "designer" && (
                  <motion.div
                    layoutId="check"
          ...
          >
            <button
              ...
              className="text-sm text-muted-foreground mb-4 self-start"
            >
              ← Back
            </button>
            ...
            <div className="space-y-4 mb-6">
              ...
              {mode === "signin" && (
                <button className="text-xs text-primary font-medium self-end block ml-auto">
                  Forgot password?
                </button>
              )}
            </div>
            ...
            <div className="mt-auto space-y-3">
              <motion.button
                ...
                className="w-full py-4 rounded-2xl bg-primary text-primary-foreground font-bold text-sm tracking-wide"
              >
                {mode === "signin" ? "Sign In" : "Create Account"}
              </motion.button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="flex items-center justify-center gap-2 py-3 rounded-xl bg-card border border-border text-sm text-foreground font-medium">
