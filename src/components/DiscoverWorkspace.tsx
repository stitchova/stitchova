...
const DiscoverWorkspace = () => {
  ...
  return (
    <div className="hidden lg:block px-8 pt-6 pb-16">
      ...
      {/* Categories */}
      <nav className="sticky top-16 z-30 flex items-center gap-1.5 rounded-full solid-panel p-1.5 w-fit my-5 backdrop-blur-xl">
        {categories.map((cat) => (
        ...
        {showFeatured && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <motion.div whileHover={{ scale: 1.005 }} onClick={() => navigate(`/designer/${featured!.id}`)}
              ...
              <SmartImage src={featured!.heroImage} alt={featured!.name}
            ...
            {filtered.map((d, i) => (
              <motion.div key={d.id} layout
                ...
                <div className="relative h-56">
                  <SmartImage src={d.heroImage} alt={d.name} containerClassName="w-full h-full"
